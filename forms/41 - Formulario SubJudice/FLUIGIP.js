	...
	this.setWriteFieldByState = function() {
		$(".form-control").each(function(){
			...
			if (typeof states != 'undefined'){
				states = (typeof states == "number" ? states.toString()  : states);
				states = states.split(",")
				if(states.indexOf(getWKNumState().toString()) != -1 || states.indexOf(getWKNumState()) != -1 || states.indexOf("*") != -1){
					FLUIGIP.execChangeFields($(this), false);
				}
			}
		});
...
var tdizoom = (function () {
	...
	return {
		 ...
		 * @param {*} filterOption Habilita select para filtrar por campo especifico
		...
		open: function (dataset, fields, resultfields, title, filters, type, likefield, likevalue, searchby, filterOption, arrFieldsForFilterCustom) {
			...
			//Colocando campos de retorno para select de possiveis filtros
			if(filterOption)
			{
				var Colunas	= fields.split(",");
				var select = "<div class='col-md-2 fs-no-padding'><select id='campoFiltro' name='campoFiltro' class='form-control'> <option value='Selecione'>Selecione</option>";                                                             
				//arrFieldsForFilter
				if(arrFieldsForFilterCustom != "" && arrFieldsForFilterCustom != null && arrFieldsForFilterCustom != undefined)
				{
					var ColunasCustom	= arrFieldsForFilterCustom.split(",");
					for (var a = 0; a < ColunasCustom.length; a++) 
					{
					    var key 	= ColunasCustom[a];
						var value 	= ColunasCustom[a+1];
				...
				select += " </select> </div>";
				html += select;
				html += "<div class='col-md-10 fs-no-padding'><input autocomplete='off' type='text' class='form-control' id='search' value='" + $("#" + type).val() + "' placeholder='Selecione o campo de filtro, digite o texto e utilize o <Enter> para buscar'></div>";
				html += "</div>";
			}
			else
			{
				html += "<div class='col-md-12 fs-no-padding'><input autocomplete='off' type='text' class='form-control' id='search' value='" + $("#" + type).val() + "' placeholder='Digite o texto e utilize o <Enter> para buscar'></div>";
				html += "</div>";
			}	
			...
			var zoommodal = FLUIGC.modal({
				...
				id: 'modal-zoom-' + type,
				actions: [{
					'label': 'Carregar todos registros',
					'classType': 'zoom-carregar-todos-registros btn-danger',
					'autoClose': false,
				},
				          {
					'label': 'Selecionar',
				...
				if(filterOption)
					$(".zoom-carregar-todos-registros").show();
				else
					$(".zoom-carregar-todos-registros").hide();
				...
				if (err) {
					...
					var urlrequest = function (carregaTodosRegistros) {
						...
						if (filters != null && filters.length > 0) {
							//Filtro para nao impactar outros zoons e ter a sinalizacao opcao de filtro ativa
							
							if(carregaTodosRegistros == "TODOS-REGISTROS")
								filters += ",todosRegistros,todosRegistros";
							else
							{
								if(filterOption)
									filters += ",limitRowsReturn,true";
							}	
							
							json.filterFields = trimarray(filters.split(","));
						}
						else
						{//Casos onde nao tem nenhum filtro inicial
							//Filtro para nao impactar outros zoons e ter a sinalizacao opcao de filtro ativa
							if(carregaTodosRegistros == "TODOS-REGISTROS")
								filters += ",todosRegistros,todosRegistros";
							else
						...
						if (searchValue && searchValue.length > 0) {
							...
							if(filterOption)
							{
								var CampoFiltro	= $("#campoFiltro").val();
								if(CampoFiltro != "Selecione")
								{
									searchby 			= CampoFiltro;
					...
					var finished = true;
					var dosearch = function (carregaTodosRegistros) {
						finished = false;
						var url = urlrequest(carregaTodosRegistros);
						$(".table-zoom > tbody").html("");
					...
					$('.zoom-carregar-todos-registros').click(function () {
						$("#campoFiltro").val("Selecione");
						$("#search").val("");
						dosearch("TODOS-REGISTROS");
					});
...
var tdizoomCheck = (function(){
	...
	return {
		open: function(dataset, fields, resultfields, title, filters, type, likefield, likevalue, searchby) {
			...
			html += "<span class='input-group-addon'><span class='fluigicon fluigicon-search'></span></span>" ;
			html += "<input autocomplete='off' type='text' class='form-control' id='search' value='" + ($("#"+type).length > 0 ? $("#"+type).val() : "") + "' placeholder='Digite o texto e utilize o <Enter> para buscar'>";
			html += "</div>" ;
			if (typeof miniZoom != "undefined") {
				html += "<div class='' id='loading-zoom' style='overflow-y: auto; height: 120px;'>";
			} else {
